package de.rockbiter.thediceguy.data

import de.rockbiter.thediceguy.R

val diceImageResources = mapOf(

    "white" to intArrayOf(
        R.drawable.dice6_1,
        R.drawable.dice6_2,
        R.drawable.dice6_3,
        R.drawable.dice6_4,
        R.drawable.dice6_5,
        R.drawable.dice6_6,
    ),

    "red" to intArrayOf(
        R.drawable.dice6_1_red,
        R.drawable.dice6_2_red,
        R.drawable.dice6_3_red,
        R.drawable.dice6_4_red,
        R.drawable.dice6_5_red,
        R.drawable.dice6_6_red
    ),


    "blue" to intArrayOf(
        R.drawable.dice6_1_blue,
        R.drawable.dice6_2_blue,
        R.drawable.dice6_3_blue,
        R.drawable.dice6_4_blue,
        R.drawable.dice6_5_blue,
        R.drawable.dice6_6_blue
    ),

    "green" to intArrayOf(
        R.drawable.dice6_1_green,
        R.drawable.dice6_2_green,
        R.drawable.dice6_3_green,
        R.drawable.dice6_4_green,
        R.drawable.dice6_5_green,
        R.drawable.dice6_6_green
    ),

    "D4_white" to intArrayOf(
        R.drawable.dice4_1,
        R.drawable.dice4_2,
        R.drawable.dice4_3,
        R.drawable.dice4_4
    ),

    "D4_blue" to intArrayOf(
        R.drawable.dice4_1_blue,
        R.drawable.dice4_2_blue,
        R.drawable.dice4_3_blue,
        R.drawable.dice4_4_blue
    ),

    "D4_green" to intArrayOf(
        R.drawable.dice4_1_green,
        R.drawable.dice4_2_green,
        R.drawable.dice4_3_green,
        R.drawable.dice4_4_green
    ),

    "D4_red" to intArrayOf(
        R.drawable.dice4_1_red,
        R.drawable.dice4_2_red,
        R.drawable.dice4_3_red,
        R.drawable.dice4_4_red
    ),
)










